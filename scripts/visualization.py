...
def plot_polar_normalized_power_distribution(directions, power_per_photon, bins=30, output_file="polar_normalized_power_distribution.png"):
    ...
    # Compute maximum radiance and ensure it's not zero to avoid division errors
    max_radiance = np.max(radiance_per_sr)
    if max_radiance == 0:
        print("Warning: Maximum radiance is zero. Normalization skipped.")
        normalized_radiance = np.zeros_like(radiance_per_sr)
    else:
...
def plot_angular_distribution(angular_deviations, power_per_photon, bins=10, output_file="angular_distribution_power.png"):
    ...
    # Ensure the histogram starts at 0.0 for the cumulative distribution
    cumulative_power_bins = np.cumsum(power_bins)
    ...
    ax1.set_xlabel("Angular Deviation (degrees)")
    ...
    ax2 = ax1.twinx()
    ax2.plot(
        bin_edges[:-1] + np.diff(bin_edges) / 2,  # Midpoints of bins
        cumulative_power_bins,
        color='red',
        marker='o',
        alpha=0.8,
...
def plot_power_azimuth(directions, power_per_photon, bins=30, output_file="power_azimuth.png"):
    ...
    # Ensure cumulative energy starts at 0.0
    cumulative_energy = np.insert(np.cumsum(energy_per_bin), 0, 0.0)

    # Azimuth bin centers
    azimuth_centers = (azimuth_edges[:-1] + azimuth_edges[1:]) / 2

    # Include 0.0 at the start for cumulative energy plot
    cumulative_centers = np.insert(azimuth_centers, 0, 0.0)
    ...
    ax2 = ax1.twinx()
    ax2.plot(cumulative_centers, cumulative_energy, marker='o', color='red', alpha=0.8, label='Cumulative Energy')
    ax2.set_ylabel("Cumulative Power (W)", color='red')
    ax2.tick_params(axis='y', labelcolor='red')
...
def plot_power_zenith(directions, power_per_photon, bins=30, output_file="power_zenith.png"):
    ...
    # Ensure cumulative power starts at 0.0
    cumulative_power = np.insert(np.cumsum(power_per_bin), 0, 0.0)

    # Zenith bin centers
    zenith_centers = (zenith_edges[:-1] + zenith_edges[1:]) / 2

    # Include 0.0 at the start for cumulative power plot
    cumulative_centers = np.insert(zenith_centers, 0, 0.0)
    ...
    ax2 = ax1.twinx()
    ax2.plot(cumulative_centers, cumulative_power, marker='o', color='red', alpha=0.8, label='Cumulative Power')
    ax2.set_ylabel("Cumulative Power (W)", color='red')
    ax2.tick_params(axis='y', labelcolor='red')
...
def plot_power_zenith_asymmetric(directions, power_per_photon, bins=30, output_file="power_zenith_asymmetric.png"):
    ...
    # Add 0.0 to cumulative power arrays
    cumulative_power_upward = np.insert(np.cumsum(power_upward), 0, 0.0)
    cumulative_power_downward = np.insert(np.cumsum(power_downward), 0, 0.0)
    ...
    ax1.set_yticks(yticks)  # Explicitly set the ticks
    ax1.set_yticklabels([f"{abs(tick):.0f}" for tick in yticks])

    # Plot cumulative power on the same axis
    ax2 = ax1.twinx()
    ax2.plot(np.linspace(0, 90, len(cumulative_power_upward)), cumulative_power_upward, marker='o', color='green', alpha=0.8, label='Cumulative Upward Power')
    ax2.plot(np.linspace(0, 90, len(cumulative_power_downward)), -cumulative_power_downward, marker='o', color='red', alpha=0.8, label='Cumulative Downward Power')
